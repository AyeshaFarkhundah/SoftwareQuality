      ...
      <td ><table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">
          <tr>
            <td class=\"banner\" valign=\"top\"><table width='100%' border='0' cellspacing='0' cellpadding='0'>
                <tr>
                  <td align=\"left\">&nbsp;</td>
                  <td align=\"right\"><div class=\"user_info\">".date('l').' '.date(Preferences('MONTH')).', '.date(Preferences('DAY')).', '.date(Preferences('YEAR'))." &nbsp;&nbsp;|&nbsp;&nbsp;".User('NAME')." &nbsp;&nbsp;|&nbsp;&nbsp; <a href='index.php?modfunc=logout' class='logout'>Log Out</a>";
/****************************************************************************************/
...
if(isset($_REQUEST['cpv_id_miss_attn']))
{
  ...
  $mp_for_cp=DBGEt(DBQuery('SELECT cp.marking_period_id FROM course_periods cp,course_period_var cpv WHERE cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID AND cpv.ID='.$_SESSION['CpvId']));
  $mp_for_cp=$mp_for_cp[1]['MARKING_PERIOD_ID'];
}
$QI = DBQuery("SELECT cpv.ID,cp.COURSE_PERIOD_ID,cp.MARKING_PERIOD_ID,cp.COURSE_ID,cp.TITLE,cp.SCHOOL_ID,cpv.PERIOD_ID FROM course_periods cp,course_period_var cpv WHERE cp.SYEAR='".UserSyear()."' AND cp.COURSE_PERIOD_ID=cpv.COURSE_PERIOD_ID AND cp.SCHOOL_ID='".UserSchool()."' AND cp.COURSE_ID='".UserCourse()."' AND (TEACHER_ID='".User('STAFF_ID')."' OR SECONDARY_TEACHER_ID='".User('STAFF_ID')."') AND (MARKING_PERIOD_ID IN (".GetAllMP($allMP,UserMP()).((isset($mp_for_cp) && $mp_for_cp && $_REQUEST['back']!='Y')?','.$mp_for_cp:'').") OR (MARKING_PERIOD_ID IS NULL)) group by (cp.COURSE_PERIOD_ID)");
$RET = DBGet($QI);
...
foreach($RET as $period)
{
  ...
  $all_cpv_ids=DBGet(DBQuery('SELECT ID FROM course_period_var WHERE COURSE_PERIOD_ID='.$period[COURSE_PERIOD_ID]));
  
     echo "<OPTION id=$period[COURSE_PERIOD_ID] value=$period[ID]".((CpvId()==$period['ID'] || (count($all_cpv_ids)>0 && in_array(CpvId(),$all_cpv_ids)==true) )?' SELECTED':'').">".$period['TITLE']." - ".$period_det['PERIOD_NAME']." - ".$period_det['DAYS']."</OPTION>";
...
echo '<div id="showhelp" style="padding-top:33px;"><a href="http://support.os4ed.com" target="_new"><b>Support Portal</b></a></div></td></tr></table>';
