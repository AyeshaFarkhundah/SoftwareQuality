...
Warehouse('header');
	...
	echo "<BODY onLoad='document.loginform.USERNAME.focus();  delete_cookie(\"dhtmlgoodies_tab_menu_tabIndex\"); get_ck();'>";
	echo "";
          ...
          <td class='content'><table width='100%' border='0' cellspacing='0' cellpadding='0'>
              <tr>
				...
				echo"<table border='0' width='100%' cellspacing='2' cellpadding='2' align=center>
                    ...
                    if(isset($_COOKIE['remember_me_name'])) $name=mysqli_real_escape_string($cont, strip_tags(trim($_COOKIE['remember_me_name']))); 
                    if(isset($_SESSION['fill_username'])) 
                    {
                    $name=$_SESSION['fill_username']; 
                    unset($_SESSION['fill_username']);
                    }
                   echo "<td width='60%' colspan='2'><input name='USERNAME' id='USERNAME' type='text' class='login_txt' value=$name></td>
                  </tr>
                  <tr>
                    <td align='right'>Password :</td>";
                    if(isset($_COOKIE['remember_me_pwd']))  $pwd=mysqli_real_escape_string($cont, strip_tags(trim($_COOKIE['remember_me_pwd']))); 
                    echo "<td colspan='2'><input name='PASSWORD' class='login_txt' id='PASSWORD' type='password' AUTOCOMPLETE = 'off' value=$pwd ></td>
                  </tr>";?>
                 <tr style="white-space: nowrap;">
                     <td></td>
                     <td style="width:50px"><input type="checkbox" name="remember" id="remember" value="Y" <?php if(isset($_COOKIE['remember_me_name'])) { echo 'checked="checked"'; } else { echo ''; } ?> />Remember Me</td>
                     <td><input name='log' type='submit' class='login' value='' onMouseDown=set_ck();Set_Cookie('dhtmlgoodies_tab_menu_tabIndex','',-1) /></td>
                 </tr>
                <?php
